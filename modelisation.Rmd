...
for (variety in levels(sims$sim)) {
  ...
  print(paste('La profondeur max du système racinaire=',max(subset(sims,sim==variety)$Z1),'[mm]'))
  print(paste0('Le système racinaire s étend de',min(subset(sims,sim==variety)$X1),'[mm] à ',max(subset(sims,sim==variety)$X1),'[mm] soit une largeur totale de :', max(subset(sims,sim==variety)$X1)-min(subset(sims,sim==variety)$X1),'[mm]'))
  print(paste('Le diamètre moyen des racines est =',mean(subset(sims, sim==variety)$Diam)))
}
```
...
```{r, echo=TRUE}
ArchiSimple(par=params_maize,exporttype = 1,time = 40)
```
